...
export function JavalinGradlewLesson() {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="px-6 sm:px-12 lg:px-24 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          <Section title="Limpiar y construir el proyecto">

            <Step number={1} title="Verificar el .env y limpiar dependencias">
              <p className="text-muted-foreground text-sm leading-relaxed mb-4">
                Primero verifica que el archivo <strong className="text-foreground">.env</strong>{' '}
                se haya creado correctamente. Luego ejecuta el siguiente comando para limpiar
                dependencias y archivos innecesarios que no pertenecen a la API:
              </p>

              <CommandBlock command="./gradlew clean" />

              <div className="grid sm:grid-cols-2 gap-4 mt-4">
                <Screenshot
                  src={`${BASE}/aws39.png`}
                  alt="Archivo .env verificado en la consola"
                  caption="Verificación del archivo .env"
                />
                <Screenshot
                  src={`${BASE}/aws40.png`}
                  alt="./gradlew clean ejecutado"
                  caption="Limpieza de dependencias completada"
                />
              </div>
            </Step>

            <Step number={2} title="Construir el ejecutable de la API">
              <p className="text-muted-foreground text-sm leading-relaxed mb-4">
                Este proceso compila el código y genera todos los archivos necesarios para
                que la API pueda ejecutarse correctamente en cualquier entorno. Puede tardar
                algunos minutos:
              </p>

              <CommandBlock command="./gradlew build" />

              <Callout type="warning">
                Si el build falla, revisa que el archivo <strong className="text-foreground">.env</strong>{' '}
                tenga los valores correctos y que la instancia de base de datos esté activa
                y accesible. Un error de conexión durante el build puede interrumpir el proceso.
              </Callout>
            </Step>

            <Step number={3} title="Ejecutar la API">
              <p className="text-muted-foreground text-sm leading-relaxed mb-4">
                Una vez que el build se complete correctamente y sin errores, ya puedes
                ejecutar la API. El archivo <strong className="text-foreground">gradlew</strong>{' '}
                se encarga de manejar la compilación y ejecución de forma automatizada,
                asegurando que todas las dependencias estén configuradas:
              </p>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-4">
                <Screenshot
                  src={`${BASE}/aws41.png`}
                  alt="./gradlew build completado exitosamente"
                  caption="Build completado sin errores"
                />
                <Screenshot
                  src={`${BASE}/aws42.png`}
                  alt="API ejecutándose con ./gradlew run"
                  caption="API corriendo correctamente"
                />
              </div>

              <Callout type="tip">
                Si la API inicia correctamente verás en la consola el puerto en el que está
                escuchando (por ejemplo <strong className="text-foreground">Listening on port 8080</strong>).
                Puedes probarlo pegando{' '}
                <strong className="text-foreground">http://&lt;tu-ip&gt;:8080</strong> en el navegador.
              </Callout>
            </Step>
