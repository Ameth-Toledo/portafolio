...
@Component({
  selector: 'app-despliegue-backend-aws4',
  standalone: true,
  imports: [CardTecnologyComponent, ImagesComponent, TecnologiasComponent, ComandCopyComponent],
  templateUrl: './despliegue-backend-aws4.component.html',
  styleUrl: './despliegue-backend-aws4.component.css'
})
export class DespliegueBackendAws4Component {
constructor(
  ...
  sendToDeployAWS2(event: Event) {
    event.preventDefault();
    this.router.navigate(['despliegue/backend/aws/ec2/id=6']).then(() => {
  ...
  sendToDeployAWS(event: Event) {
    event.preventDefault();
    this.router.navigate(['despliegue/backend/aws/ec2/id=4']).then(() => {
      this.viewportScroller.scrollToPosition([0, 0]);
    });
  }
}
